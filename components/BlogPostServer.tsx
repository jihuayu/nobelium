    ...
    <Link href={`${blogPath}/${post.slug}`} prefetch={false} className="group block">
      <article key={post.id} className="mb-10 md:mb-12">
        <header className="flex flex-col justify-between md:flex-row md:items-baseline md:gap-6">
          <h2 className="text-lg md:text-2xl font-serif font-medium mb-1.5 cursor-pointer text-stone-900 dark:text-stone-100 tracking-tight underline-offset-[5px] decoration-1 decoration-stone-300 dark:decoration-stone-600 group-hover:underline">
            {post.title}
          </h2>
